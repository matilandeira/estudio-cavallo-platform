...
import { isCarCompleted, normalizeCarStatus, whatsappLinkCarDocsReady, whatsappLinkCoordinateSigning, whatsappLinkRequestDocumentation } from "../lib/businessLogic.js";
...
export default function Cars({ cars, documentsReadyToSchedule, simpleMode, highlightId, searchSeed }) {
  ...
  // Arriving here via global search: show just that client's cars (clearing
  // any dropdown filter that might otherwise hide the one being jumped to)
  // so useRowHighlight always has an actual rendered row to scroll to.
  useEffect(() => {
  ...
  const filtered = cars.rows
    .filter((c) =>
      !isCarCompleted(c) &&
      (!search || `${c.client} ${c.make_model} ${c.registry_number} ${c.plate_number}`.toLowerCase().includes(search.toLowerCase())) &&
      (!filters.status || normalizeCarStatus(c.status) === filters.status) &&
      (!filters.assignee || assigneeMatches(c.assignees, filters.assignee) || assigneeMatches(c.notarization_assignees, filters.assignee)) &&
      (!urgentOnly || isUrgent(c.reminder_date, normalizeCarStatus(c.status)))
    )
    .sort(byPriority);
  const urgentCount = cars.rows.filter((c) => !isCarCompleted(c) && isUrgent(c.reminder_date, normalizeCarStatus(c.status))).length;
  ...
  const update = (id, patch, opts) => {
    ...
    if (allGreen(next) && !allGreen(car) && normalizeCarStatus(next.status) === "Pending") {
  ...
  /* Keeps "Documentos prontos para agendar" in sync with every car currently
     "Pronto para firma" — reactively, not via a one-way `ready_to_schedule`
     flag set once inside update(). That flag can't tell the difference
     between "already queued" and "was queued once, then the queue entry got
     ...
     reappears, even after re-selecting "Pronto para firma" (this is exactly
     what happened to one car in seed.sql: seeded directly with status
     "Ready to Sign" and ready_to_schedule true, but with no matching queue
     row). Checking live membership instead — every time cars or the queue
     change, including realtime updates from other staff — makes this
     self-healing. Status is normalized (see normalizeCarStatus) so an older
     row still carrying one of the retired statuses is judged the same way
     a current one would be.

     The same pass also removes a car's queue entry once it's no longer
     "Pronto para firma" — e.g. moved back to "Pendiente" by mistake, or
     straight to "Finalizado" — so the queue doesn't keep showing a car that
     isn't actually ready to schedule anymore. Matching by client+make/model
     (there's no car_id column on documents_ready_to_schedule) means this
     only ever touches entries that clearly came from a real car; manually
     added entries or ones from Inmuebles never coincidentally match. */
  ...
  useEffect(() => {
    ...
    const missing = cars.rows.filter(
      (c) =>
        normalizeCarStatus(c.status) === "Ready to Sign" &&
    ...
    const carsByKey = new Map(cars.rows.map((c) => [`${c.client} ${c.make_model}`, c]));
    const stale = queued.filter((q) => {
      const matchedCar = carsByKey.get(`${q.client} ${q.description}`);
      return matchedCar && normalizeCarStatus(matchedCar.status) !== "Ready to Sign";
    });
    stale.forEach((q) => documentsReadyToSchedule.removeRow(q.id));
    ...
    <div className="ec-fade">
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        <FilterBar filters={filters} setFilters={setFilters} options={[
          { key: "status", label: "Estado", values: CAR_STATUSES.filter((s) => s !== "Completed"), labelFor: (v) => translate(CAR_STATUS_LABELS, v) },
      ...
      <div data-tour="list" style={{ display: "grid", gap: 10 }}>
        ...
        {filtered.map((a, idx) => {
          const carStatus = normalizeCarStatus(a.status);
          const dueReminder = carStatus === "Pending" && a.reminder_date && a.reminder_date <= todayISO();
          return (
          <div key={a.id} id={`row-${a.id}`} className={activeHighlight === a.id ? "ec-card ec-highlight" : "ec-card"} style={{ padding: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <select data-tour={idx === 0 ? "status-select" : undefined} className="ec-select" style={{ width: "auto" }} value={carStatus} onChange={(e) => update(a.id, { status: e.target.value })}>
            ...
            {carStatus === "Pending" && (
              <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px dashed ${C.line}`, display: "flex", flexWrap: "wrap", gap: 8 }}>
                ...
                {/* No longer gated on a separate "Returned from Registry" status
                    (retired — see normalizeCarStatus) — available any time the
                    car is still Pendiente, since staff may need to send this
                    notice at any point before the car is ready to sign. */}
                {whatsappLinkCarDocsReady(a) ? (
                  <a href={whatsappLinkCarDocsReady(a)} target="_blank" rel="noopener noreferrer" className="ec-btn" style={{ textDecoration: "none", background: "#2CA043", borderColor: "#2CA043" }}>
                    Avisar WhatsApp {a.financed ? "(financiado)" : "(prontos)"}
                  </a>
                ) : (
                  <span className="ec-btn" style={{ opacity: .55, cursor: "default", background: "#2CA043", borderColor: "#2CA043" }} title="Cargá un teléfono para habilitar el envío por WhatsApp">
                    Avisar WhatsApp {a.financed ? "(financiado)" : "(prontos)"} (sin teléfono)
                  </span>
                )}
              </div>
            )}
            {carStatus === "Ready to Sign" && (
