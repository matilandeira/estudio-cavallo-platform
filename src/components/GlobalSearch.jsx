...
import { isCarCompleted, isDocumentCompleted, isPropertyCompleted } from "../lib/businessLogic.js";
...
/* Global Ctrl/Cmd+K search across cars, documents, properties and the
   ...
   Supabase round trip, no debounce needed.

   Two ways to act on a search:
   - Pick a specific result -> onNavigate(tab, id, query): switches tabs,
     seeds that tab's own local search box with `query` (so the row is
     guaranteed to actually be rendered, not hidden behind some other
     filter), and highlights/scrolls to that exact row (see useRowHighlight
     in SharedUI.jsx).
   - Press Enter, or click "Ver todos los resultados" -> submitSearch():
     same navigate-and-seed-search behavior, but with no specific row to
     highlight (id is null) — it just filters the target tab's whole list
     by the query. The target tab is whichever one the query already
     matches on `currentTab` (staying put), or otherwise the first
     category with results, in the fixed order the groups are always
     listed in (Autos, Inmuebles, Documentos, Agenda).

   Either way, results are restricted to whatever each target tab can
   actually show — completed cars/documents/properties are excluded (those
   tabs never display completed items, no matter the filter state) and past
   signing appointments are excluded (the Agenda panel only ever lists
   upcoming ones) — otherwise a result could navigate to a tab where the row
   structurally can never appear. */
export default function GlobalSearch({ cars, documents, properties, signingAppointments, currentTab, onNavigate }) {
  ...
  const groups = useMemo(() => {
    ...
    const autos = (cars?.rows || [])
      .filter((c) => !isCarCompleted(c) && matches(c.client, c.registry_number, c.make_model, c.plate_number))
      .slice(0, MAX_PER_GROUP)
      .map((c) => ({
        id: c.id, tab: "cars", query: c.client || "",
    ...
    const inmuebles = (properties?.rows || [])
      .filter((p) => !isPropertyCompleted(p) && matches(p.client, p.registry_number, p.notes, p.next_action))
      .slice(0, MAX_PER_GROUP)
      .map((p) => ({
        id: p.id, tab: "properties", query: p.client || p.registry_number || "",
        title: p.client || "Cliente sin nombre",
        subtitle: p.registry_number ? `Padrón ${p.registry_number}` : "—",
      }));

    const documentos = (documents?.rows || [])
      .filter((d) => !isDocumentCompleted(d) && matches(d.client, d.document_type, translate(DOCUMENT_TYPE_LABELS, d.document_type)))
      .slice(0, MAX_PER_GROUP)
      .map((d) => ({
        id: d.id, tab: "documents", query: d.client || "",
        title: d.client || "Cliente sin nombre",
        subtitle: translate(DOCUMENT_TYPE_LABELS, d.document_type) || "—",
      }));

    // Only upcoming appointments — the Agenda panel on Inicio never lists
    // past ones, so a past match could never actually be shown/highlighted there.
    const agenda = (signingAppointments?.rows || [])
      .filter((a) => a.appointment_date >= today && matches(a.client, a.description))
      .slice(0, MAX_PER_GROUP)
      .map((a) => ({
        id: a.id, tab: "home", query: a.client || "",
  ...
  // "Most relevant tab": prefer wherever the user already is if the query
  // matches something there, otherwise the first non-empty category in the
  // groups' fixed display order (Autos, Inmuebles, Documentos, Agenda).
  const primaryGroup = groups.find((g) => g.items[0]?.tab === currentTab) || groups[0] || null;

  const pick = (item) => {
    onNavigate(item.tab, item.id, item.query);
    setOpen(false);
  };

  const submitSearch = () => {
    const q = query.trim();
    if (!q || !primaryGroup) return;
    onNavigate(primaryGroup.items[0].tab, null, q);
    setOpen(false);
  };
        ...
        <div className="ec-search-overlay" style={{ position: "fixed", inset: 0, background: "rgba(30,42,36,.5)", zIndex: 300, display: "flex", justifyContent: "center", paddingTop: "12vh" }}>
          ...
          >
            ...
            <div className="ec-scroll" style={{ overflowY: "auto" }}>
              ...
              {totalResults > 0 && primaryGroup && (
                <button
                  onClick={submitSearch}
                  onMouseDown={(e) => e.preventDefault()}
                  className="ec-search-result"
                  style={{ width: "100%", textAlign: "center", background: "none", border: "none", borderTop: `1px solid ${C.line}`, cursor: "pointer", padding: "10px 16px", fontSize: 12.5, fontWeight: 700, color: C.wax, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
                >
                  Ver todos los resultados en {primaryGroup.label}
                  <span className="ec-mono" style={{ opacity: .7, fontWeight: 400, fontSize: 11 }}>Enter ↵</span>
                </button>
              )}
            </div>
          </div>
